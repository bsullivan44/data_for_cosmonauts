...
Go to [0-introduction](https://github.com/Djamil17/data_for_cosmonauts/tree/master/0-introduction) and read the README.md to look for instructions on how to install.
...
![Image of Elements of Data Science](https://github.com/Djamil17/data_for_cosmonauts/blob/master/elements-of-data-science.jpg)
...
They begin with an example where we pretend to be statistical consultants for some firm. I won't entrtain that scenario. Instead, imagine we are data scientists attempting to understand who, what kinds of persons, are most likely to get cancer. More importantly, we would like to find the features, or relations, that allow us to infer who is most likely to get cancer. 
In order to do so we get data and process it in the form of a table. Here we might have data with columns such as Age, Height, Weight, Systolic Blood Pressure, Diatolic Blood Pressure, Medical History Count, Previous Cancer Occurences, and Cancer Occured. We would use statistical learning to split this data into a feature matrix X1, X2, X3 and target vector y and to model a function(X1,X2, ...Xn) of these *inputs*, * independent variables*, or *predictors* to infer the output, dependent variables, or *response variable* y.Some of these variables are *numerical* and others *categorical*. Numerical is simple enough to understand. Some are categorical, that means they have a limited set of values. For instance, "IS MARRIED" is a *two-valued or binary* categorical variable. "DAY OF THE WEEK" could be considered a varaible with 7 values. It is a *multi-valued* categorical variable.In simple terms, we will "do machine-learning" to use variables like Age, Weight etc. to predict if someone will get cancer. 

The book concludes "in essence statistical learning refers to a set of approaches for estimating ***f***". However, this is patently false and it is incredible that it is stated. We will come to understand why. But the simple example provides a good paradigm for the sort of work done in machine-learning.
